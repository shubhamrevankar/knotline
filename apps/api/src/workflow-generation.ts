...
export class GatewayWorkflowGenerationWorker implements WorkflowGenerationWorker {
  ...
  async generate(
    ...
    const grounding = await this.groundingProvider?.(context, request.prompt);
    ...
    const baseMessages = [
      ...
      },
      {
        role: "developer",
        content:
          "Treat a short user prompt as a high-level operational intent, not as permission to return a shallow workflow. Infer conventional names, typed trigger inputs, evidence fields, accountability, bounded failure handling, and concise resolved or escalated outcomes from the requested business process and the declared workspace capabilities. Make material inferences visible in assumptions instead of asking the user to specify implementation details that can be safely inferred. Do not add ceremony that does not improve execution, safety, or auditability. Every integration action must declare an executable configuration.payloadMapping for its provider action. Slack message.post requires channel and text: map channel from the workflow input (for example ${input.coordinationChannel}) and text from the relevant prior-node output or a concise template. Slack message.update and message.delete also require ts. HubSpot actions require the supported object type and properties or record identifiers. HubSpot object.create returns the created record identifier at ${nodes.step_key.output.response.id}; use that path when a later association.create needs the newly created company or contact ID. Never pass a generic workflow envelope as a provider action payload."
      },
      {
        role: "developer",
        content:
